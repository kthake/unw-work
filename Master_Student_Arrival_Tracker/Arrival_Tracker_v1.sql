select distinct
       a1.PERSON_UID,
       a1.DATA_ORIGIN,
       decode(substr(a1.DATA_ORIGIN,1,1),1,'Registered Trad',
                                         2,'Registered PSOC',
                                         3,'Deposited Potential PSOC',
                                         4,'Deposited but not registered',
                                         5,'Not returned') "DATA_ORIGIN_DESC",
       a1.ID,
       a1.NAME_LFMI,
       a1.GENDER,
       a1.BIRTH_DATE,
...
--------------------------------------------------------------------------START a1
  ...
  select a.PERSON_UID,
         ...
         nvl(NWC.Z_FORMAT_NAME_FNC(NWC.Z_GET_POSITION_INCUMBENT_FNC(b.RD_POSITION),'FL'),' ') "RD",
         nvl(NWC.Z_FORMAT_NAME_FNC(act.PERSON_UID,'LFMI'), ' ') "RA_NAME",
         nvl(h1.ROOM,' ') "RA_ROOM",
...
--------------------------------------------------------------------------START a
    ...
    where z.ACADEMIC_PERIOD_MOD = '20162'
          and ( z.COLLEGE = 'TR'
    ...
    where substr(aa.ACADEMIC_PERIOD,1,5) = '20162' --------------
          and ( aa.COLLEGE = 'TR'
    ...
    where z.ACADEMIC_PERIOD_MOD = '20161'
          and z.COLLEGE = 'TR'
          and z1.PERSON_UID is null
          and z.PRIMARY_PROGRAM_IND = 'Y'
          and z.WITHDRAWN_IND = 'N'
          and ( z.COMPLETION_IND = 'N'
                or z1.PERSON_UID is null )
